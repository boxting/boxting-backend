...
router.post('/:id/token/add/collaborator', authenticateToken, authenticateRole([RoleEnum.ADMIN, RoleEnum.ORGANIZER]), handleAddCollaborator)
router.post('/token/create', authenticateToken, authenticateRole([RoleEnum.ORGANIZER]), handleCreateEventWithToken)
router.get('/token/get/:id', authenticateToken, handleGetEventByIdWithToken)
